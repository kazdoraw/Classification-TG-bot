"""
Train Primary Classifier using Vision Transformer (ViT-B/16).

Uses transfer learning from pretrained ViT for comparison with CNN models.
"""
...
CONFIG = {
    ...
    'weight_decay': 1e-4,
    'patience': 4,  # Early stopping patience - reduced
    'device': DEVICE,
    'num_workers': 4,
    'pin_memory': True,
}
...
def get_data_transforms():
    """Get data augmentation transforms for ViT."""
...
def create_model(num_classes, device):
    """Create ViT-B/16 model with transfer learning."""
    # Load pretrained ViT-B/16
    weights = ViT_B_16_Weights.IMAGENET1K_V1
    model = models.vit_b_16(weights=weights)
    
    # Freeze backbone (optional - uncomment for faster training)
    ...
    logger.info(f"Model created: ViT-B/16 (pretrained)")
    logger.info(f"Modified head: {num_features} -> {num_classes}")
...
def main():
    ...
    logger.info("="*60)
    logger.info("Training Primary Classifier - Vision Transformer")
    logger.info("="*60)
    
    MODELS_DIR.mkdir(parents=True, exist_ok=True)
    output_dir = MODELS_DIR / "vit"
    output_dir.mkdir(exist_ok=True)
    
    device = torch.device(CONFIG['device'])
    logger.info(f"Using device: {device}")
    logger.info("⚠️  ViT is computationally expensive - training may be slow on CPU")
    ...
    logger.info(f"Batch size: {CONFIG['batch_size']} (smaller for ViT)")
